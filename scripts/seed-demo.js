...
const ISSUES = [
  { subject: 'SSO login loops back to the sign-in page', severity: 'P1', category: 'Authentication',
    ...
    steps: 'Cleared cookies, tried incognito, confirmed the user exists and is active. Reproduced on two machines.',
    impact: 'Whole team locked out since 08:00. They cannot dispatch drivers.' },
  { subject: 'Monthly invoice charged twice', severity: 'P2', category: 'Billing',
    ...
    steps: 'Confirmed both charges in the billing portal. No duplicate subscription on the account.',
    impact: 'Customer is disputing with their bank if not resolved today.' },
  { subject: 'CSV export truncates at 1,000 rows', severity: 'P3', category: 'Reporting',
    ...
    steps: 'Reproduced on my own account. Confirmed the UI shows no limit notice.',
    impact: 'Customer is doing a quarterly reconciliation and believes data is missing.' },
  { subject: 'Webhook deliveries stopped overnight', severity: 'P1', category: 'Integrations',
    ...
    steps: 'Checked the endpoint with curl, confirmed 200. Delivery log in the dashboard is empty for the period.',
    impact: 'Their downstream CRM is not receiving new leads.' },
  { subject: 'How do I bulk-reassign contacts to a new owner?', severity: 'P4', category: 'How-to',
    ...
    steps: 'Checked the help centre; there is no article covering bulk reassignment.',
    impact: 'Manual workaround would take them several hours.' },
  { subject: 'SSO login loops after password reset', severity: 'P2', category: 'Authentication',
    ...
    steps: 'Confirmed the redirect URI on their identity provider does not match ours.',
    impact: 'Two users affected, others can still log in.' },
  { subject: 'Calendar invites arriving in the wrong timezone', severity: 'P3', category: 'Calendar',
    ...
    steps: 'Checked account timezone and the individual user timezone. Both correct.',
    impact: 'Three no-shows this week.' },
  { subject: 'Report page takes 40+ seconds to load', severity: 'P3', category: 'Performance',
    ...
    steps: 'Reproduced. Other accounts load in under 3 seconds. This account has ~400k records.',
    impact: 'Their ops lead has stopped using reporting entirely.' },
];
...
for (let i = 0; i < 24; i++) {
  ...
  const { ticket } = createTicket({
    ...
    stepsTaken: issue.steps,
    customerImpact: issue.impact,
    severity: issue.severity,
    category: issue.category,
    escalatedBy: pick(agents),
    source: 'seed',
  });
